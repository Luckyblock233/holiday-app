...
export function calcEarnedMinutes({
  ...
  const exerciseOk = todayRecord.exercise_minutes >= 20;
  ...
  if (isParentChecked) {
    // 额外阅读，且多写1条笔记，可重复累计
    // “多写1条”理解为：超过最低要求的笔记数（>=2）
    const extraNotes = Math.max(0, todayNotesCount - 1);
    bonusReading = extraNotes * 15;

    // 额外运动≥10，可重复累计
    const extraExercise = Math.max(0, todayRecord.exercise_minutes - 20);
    bonusExercise = Math.floor(extraExercise / 10) * 10;
  }
